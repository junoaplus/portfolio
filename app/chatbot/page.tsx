...
const companyConfigs: Record<string, CompanyConfig> = {
  ...
  ably: {
    ...
    description: 'AI Engineer - 생성형 AI 서비스 개발',
  },
  nuua: {
    name: '누아',
    color: 'green-500',
...
저는 LLM/RAG 시스템 구축 경험과 실제 사업 운영 경험을 모두 가진 지원자입니다. AI 기술 역량과 함께 이커머스 사업을 직접 운영하며 비즈니스 관점을 이해한 경험이 있어, 토스의 다양한 AI 서비스(금융, 커머스, 추천 등) 개발에 기술적 역량과 실무적 인사이트를 모두 제공할 수 있습니다.
...
## 왜 저는 Game N에 최적의 인재일까요?

게임과 AI의 융합에 큰 관심을 가지고 있는 개발자입니다. 사용자 경험을 중심으로 한 개인화 AI 시스템 구축 경험이 있으며, 게임 플레이어들에게 더 재미있고 편리한 경험을 제공하는 것이 목표입니다.
...
## Game N에서 AI로 게임 경험을 혁신하고 싶습니다. 궁금한 점이 있으시면 언제든 질문해주세요!`;
    ...
    } else if (companyKey === "ably") {
      return `# 에이블리 AI Engineer 황준호

**지금 이 순간 면접관님과 대화하고 있는 이 AI 시스템이 제가 구축한 멀티 에이전트 포트폴리오입니다**

## 왜 저는 에이블리에 최적의 인재일까요?

패션에 대한 관심과 패션 이커머스 사업 경험(의류 판매), 그리고 AI 기술 역량이 모두 결합된 지원자입니다. 실제 옷을 판매해본 경험과 AI 에이전트 시스템 구축 경험을 동시에 보유하고 있어, 패션테크 기업인 에이블리에 최적화된 인재라고 확신합니다.

## 에이블리 채용 요구사항과 제 경험 매칭

🎨 **생성형 AI 서비스 백엔드**: LangGraph 멀티 에이전트 포트폴리오  
   FastAPI 기반 AI 모델 서빙 인프라 구축 경험  
   Docker 컨테이너화로 배포 환경 일관성 확보  
   👉 [이 프로젝트 자세히 보기](/ai-chatbot-portfolio)

💬 **LLM 서비스 통합 경험**: 보드게임 RAG 챗봇  
   EXAONE LLM 파인튜닝 + RAG 시스템 구축  
   QA 데이터 자동 수집/재학습 피드백 루프로 지속적 모델 개선  
   AWS EC2 기반 서비스 배포 및 운영  
   👉 [이 프로젝트 자세히 보기](/boardgame-chatbot)

⚡ **AI 모델 성능 최적화**: 에이전트 시스템 최적화  
   33초→3.4초 (90% 개선) 달성으로 컴퓨팅 리소스 최적화  
   인메모리 캐싱 + 병렬 처리로 실시간 AI 서비스 성능 보장  
   👉 [이 프로젝트 자세히 보기](/ai-chatbot-portfolio)

🔧 **ML 모델 학습/서빙**: 신문 이탈 예측  
   RandomForest, XGBoost 등 ML 모델 학습 및 서빙 파이프라인 구축  
   PyTorch 기반 모델 최적화 경험  
   👉 [이 프로젝트 자세히 보기](/newspaper-churn)

📊 **대용량 데이터 처리**: 데이트 코스 AI 추천  
   PySpark ETL로 20만→9만 장소 데이터 정제  
   비정형 데이터 처리 파이프라인 구축  
   👉 [이 프로젝트 자세히 보기](/date-recommendation)

🚀 **빠른 기술 적용 사례**: E-commerce 사업 운영  
   2023년 8월 온라인 쇼핑몰 운영하며 AI 기반 상품 추천 필요성 체감  
   커머스 도메인에 대한 실전 이해  
   👉 [사업 운영 경험 자세히 보기](/about#ecommerce)

## One Team으로 Grit하게 Impact를 만들어가고 싶습니다. 궁금한 점이 있으시면 언제든 질문해주세요!`;

    } else if (companyKey === "nuua") {
...
## "당연해야 할 것들에 도전"하는 누아와 함께하고 싶습니다. 궁금한 점이 있으시면 언제든 질문해주세요!`;
...
## 법을 사람들에게 더 가깝게 만드는 국내 1위 리걸테크의 미션에 함께하고 싶습니다. 궁금한 점이 있으시면 언제든 질문해주세요!`;
...
## "실용주의 인공지능으로 일상에 편리함을 더하는" 이스트소프트와 함께하고 싶습니다. 궁금한 점이 있으시면 언제든 질문해주세요!`;

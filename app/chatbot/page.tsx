...
export default function ChatbotPage() {
  ...
  const getCompanySpecificResponse = (question: string): string => {
    ...
    if (lowerQuestion.includes("기술") || lowerQuestion.includes("스택")) {
      const focusText =
        selectedCompany === "general" ? "전반적인 기술 스택을" : `${company.name}에서 중요하게 여기는 기술들을 중심으로`
        ...
        <div className="pt-16 min-h-screen flex items-center justify-center px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="mb-16">
              ...
              <h1 className="text-4xl font-bold text-gray-900 mb-4">AI 어시스턴트에 오신 것을 환영합니다</h1>
              <p className="text-xl text-gray-600 mb-12">어떤 방식으로 포트폴리오를 둘러보시겠어요?</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-12">
              ...
              >
                <CardContent className="p-0">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-3">일반 모드</h3>
              ...
              <Card className="bg-white border border-gray-200 hover:border-gray-300 hover:shadow-lg transition-all duration-200 p-8">
                <CardContent className="p-0">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-3">회사별 맞춤 모드</h3>
                  ...
                  <div className="space-y-3">
                    {Object.entries(companyConfigs)
                      ...
                      .map(([key, company]) => (
                        ...
                        >
                          <div className={`w-3 h-3 bg-${company.color} rounded-full mr-3`}></div>
            ...
            <div className="bg-blue-50 rounded-2xl border border-blue-100 p-6">
              <p className="text-sm text-blue-800">
                💡 <strong>팁:</strong> 언제든지 모드를 변경할 수 있습니다. 회사별 맞춤 모드에서는 해당 회사의 관심사에
                맞는 프로젝트와 기술을 중심으로 답변합니다.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 bg-${currentCompany?.color} rounded-xl flex items-center justify-center`}>
                <Bot className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-gray-900">
                  {selectedCompany === "general" ? "일반" : currentCompany?.name} 포트폴리오 AI 어시스턴트
                </h1>
            ...
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-600 font-medium">AI 에이전트 온라인</span>
              </div>
              <Badge className={`bg-${currentCompany?.color} text-white`}>
                <Sparkles className="w-3 h-3 mr-1" />
                {selectedCompany === "general" ? "일반 모드" : `${currentCompany?.name} 전용`}
              </Badge>
              <Button
                variant="ghost"
                size="sm"
        ...
        <div className="flex-1 flex flex-col">
              ...
              {messages.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <h2 className="text-2xl font-bold mb-6 text-gray-900">
                    안녕하세요!{" "}
                    {selectedCompany === "general" ? "포트폴리오 방문자님" : `${currentCompany?.name} 면접관님`}
                  </h2>
                    ...
                    {currentCompany?.focus.map((focus, index) => (
                      <div key={index} className="bg-white rounded-xl border border-gray-200 p-4">
                        <div className="flex items-center gap-3 mb-2">
                          <Zap className={`w-5 h-5 text-${currentCompany?.color}`} />
                          <span className={`font-semibold text-${currentCompany?.color}`}>{focus}</span>
                        </div>
                        <p className="text-gray-600 text-sm">관련 프로젝트 경험과 기술적 깊이</p>
                      </div>
                    ))}
                  </div>
                </div>
                            ...
                            >
                              ...
                              >
                                ...
                                {message.sources.map((source, index) => (
                                  <Badge key={index} className="text-xs bg-gray-100 text-gray-700 border-0">
                                    {source}
                                  </Badge>
                                ))}
                              </div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center gap-2">
                        <div className="flex gap-1">
                          <div className={`w-2 h-2 bg-${currentCompany?.color} rounded-full animate-bounce`}></div>
                          <div
          ...
          <div className="p-4 bg-white border-t border-gray-200 flex-shrink-0">
            <div className="flex gap-3 w-full">
              ...
              <Button variant="ghost" size="sm" className="px-3 hover:bg-gray-100 text-gray-500">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="font-semibold mb-3 text-gray-900">
                  {selectedCompany === "general" ? "전체 포트폴리오" : `${currentCompany?.name} 맞춤 정보`}
                </h3>
              ...
              <div>
                <h3 className="font-semibold mb-3 text-gray-900">시스템 정보</h3>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`text-${currentCompany?.color} font-medium`}>
                      {selectedCompany === "general" ? "일반" : `${currentCompany?.name} 전용`}
                    </span>
                  </div>
                </div>
              </div>
